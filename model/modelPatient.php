...
//        on utilise un bind value pour chaque clé nominatif
    ...
    public function changePatientContent(){
        $request = 'UPDATE  `patients` SET `lastname` = :lastname,`firstname` = :firstname,`birthdate` = :birthdate,`mail` = :mail,`phone` = :phone WHERE `id` = :id';
        $changeContent = $this->db->prepare($request);
        //
        $changeContent->bindValue(':lastname', $this->lastname, pdo::PARAM_STR);
        $changeContent->bindValue(':id', $this->id, pdo::PARAM_INT);
        $changeContent->bindValue(':firstname', $this->firstname, pdo::PARAM_STR);
        $changeContent->bindValue(':birthdate', $this->birthdate, pdo::PARAM_STR);
        $changeContent->bindValue(':phone', $this->phone, pdo::PARAM_STR);
        $changeContent->bindValue(':mail', $this->mail, pdo::PARAM_STR);
        return $changeContent->execute();
        }
}
