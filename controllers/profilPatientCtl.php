<?php
include_once '../model/modelPatient.php';
//instanciation 
...
if (isset($_POST['submit'])) {
    $changePatient = NEW patients();
    $changePatient->id = $patient->id;
    if (!empty($_POST['lastname'])) {
        if (preg_match($regexName, $_POST['lastname'])) {
            $changePatient->lastname = htmlspecialchars($_POST['lastname']);
        } else {
    ...
    if (!empty($_POST['firstname'])) {
        if (preg_match($regexName, $_POST['firstname'])) {
            $changePatient->firstname = htmlspecialchars($_POST['firstname']);
        } else {
    ...
    if (!empty($_POST['birthdate'])) {
        if (preg_match($regexBirthDate, $_POST['birthdate'])) {
            $changePatient->birthdate = htmlspecialchars($_POST['birthdate']);
        } else {
    ...
    if (!empty($_POST['phone'])) {
        if (preg_match($regexPhoneNumber, $_POST['phone'])) {
            $changePatient->phone = htmlspecialchars($_POST['phone']);
        } else {
     ...
     if (!empty($_POST['mail'])) {
        if (preg_match($regexMail, $_POST['mail'])) {
            $changePatient->mail = htmlspecialchars($_POST['mail']);
        } else {
    ...
    if (count($formError) == 0){
        if (!$changePatient->changePatientContent()){
            $formError['submit'] = 'Il y a eu un problème';
        }
    }
}
$profilPatient = $patient->getProfilPatient();